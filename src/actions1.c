...
L4090:
   ...
   * "INVENT(ORY)" (AND NO NEW VERB YET TO BE ANALYSED).  WATER AND OIL ARE ALSO
   * FUNNY, SINCE THEY ARE NEVER ACTUALLY DROPPED AT ANY LOCATION, BUT MIGHT BE
   * HERE INSIDE THE BOTTLE OR URN OR AS A FEATURE OF THE LOCATION. */
...
L5100:
  ...
  if (K != GRATE)
    return (8);
L5110:
  if (K == DWARF && ATDWRF(LOC) > 0)
    goto L5010;
  if ((LIQ(0) == K && HERE(BOTTLE)) || K == LIQLOC(LOC))
    goto L5010;
...
L5190:
  ...
  /*  STATEMENT NUMBERS IN THIS SECTION ARE 8000 FOR INTRANSITIVE VERBS, 9000
   * FOR TRANSITIVE, PLUS TEN TIMES THE VERB NUMBER.  MANY INTRANSITIVE VERBS
   * USE THE TRANSITIVE CODE, AND SOME VERBS USE CODE FOR OTHER VERBS, AS NOTED
   * BELOW. */

  /*  CARRY, NO OBJECT GIVEN YET.  OK IF ONLY ONE OBJECT PRESENT. */

L8010:
  if (ATLOC[LOC] == 0 || LINK[ATLOC[LOC]] != 0 || ATDWRF(LOC) > 0)
    return (8000);
  OBJ = ATLOC[LOC];
...
L8070:
  if (HERE(LAMP) && PROP[LAMP] == 0 && LIMIT >= 0)
    OBJ = LAMP;
  if (HERE(URN) && PROP[URN] == 1)
    OBJ = OBJ * 100 + URN;
  if (OBJ == 0 || OBJ > 100)
    return (8000);
...
L8080:
  if (HERE(LAMP) && PROP[LAMP] == 1)
    OBJ = LAMP;
  if (HERE(URN) && PROP[URN] == 2)
    OBJ = OBJ * 100 + URN;
  if (OBJ == 0 || OBJ > 100)
    return (8000);
...
L9134:
  ...
  /*  EAT.  INTRANSITIVE: ASSUME FOOD IF PRESENT, ELSE ASK WHAT.  TRANSITIVE:
   * FOOD OK, SOME THINGS LOSE APPETITE, REST ARE RIDICULOUS. */
...
L9190:
  if (AT(OBJ) || (LIQ(0) == OBJ && AT(BOTTLE)) || K == LIQLOC(LOC) ||
...
L8240:
   ...
   * ORDER. LOOK UP WD1 IN SECTION 3 OF VOCAB TO DETERMINE WHICH WORD WE'VE GOT.
...
L8332:
  ...
  /* 8335 */ for (I = 1; I <= 100; I++) {
    if (!HERE(I) || OBJSND[I] == 0 || PROP[I] < 0)
      goto L8335;
    PSPEAK(I, OBJSND[I] + PROP[I]);
    SPK = 0;
    if (I == BIRD && OBJSND[I] + PROP[I] == 8)
      DSTROY(BIRD);
  L8335: /*etc*/;
  } /* end loop */
  return (2011);
