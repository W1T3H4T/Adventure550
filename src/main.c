...
long ABB[186], ATAB[331], ATLOC[186],
    BLKLIN = TRUE, DFLAG, DLOC[7], FIXED[101], HOLDNG, KTAB[331], *LINES,
    LINK[201], LNLENG, LNPOSN, PARMS[26], PLACE[101], PTEXT[101], RTEXT[278],
    SETUP = 0, TABSIZ = 330;
char INLINE[101], MAP1[129], MAP2[129];

long ABBNUM, ACTSPK[36], AMBER, ATTACK, AXE, BACK, BATTER, BEAR, BIRD, BLOOD,
    BONUS, BOTTLE, CAGE, CAVE, CAVITY, CHAIN, CHASM, CHEST, CHLOC, CHLOC2, CLAM,
    CLOCK1, CLOCK2, CLOSED, CLOSNG, CLSHNT,
    CLSMAX = 12, CLSSES, COINS, COND[186], CONDS, CTEXT[13], CVAL[13], DALTLC,
    DETAIL, DKILL, DOOR, DPRSSN, DRAGON, DSEEN[7], DTOTAL, DWARF, EGGS, EMRALD,
    ENTER, ENTRNC, FIND, FISSUR, FIXD[101], FOOBAR, FOOD, GRATE, HINT,
    HINTED[21], HINTLC[21], HINTS[21][5], HNTMAX, HNTSIZ = 20, I, INVENT, IGO,
    IWEST, J, JADE, K, K2, KEY[186], KEYS, KK, KNFLOC, KNIFE, KQ, L, LAMP,
    LIMIT, LINSIZ = 12500, LINUSE, LL, LMWARN, LOC, LOCK, LOCSIZ = 185,
    LOCSND[186], LOOK, LTEXT[186], MAGZIN, MAXDIE, MAXTRS, MESH = 123456789,
    MESSAG, MIRROR, MXSCOR, NEWLOC, NOVICE, NUGGET, NUL, NUMDIE, OBJ,
    OBJSND[101], OBJTXT[101], ODLOC[7], OGRE, OIL, OLDLC2, OLDLOC, OLDOBJ,
    OYSTER, PANIC, PEARL, PILLOW, PLAC[101], PLANT, PLANT2, PROP[101], PYRAM,
    RESER, ROD, ROD2, RTXSIZ = 277, RUBY, RUG, SAPPH, SAVED, SAY, SCORE, SECT,
    SIGN, SNAKE, SPK, STEPS, STEXT[186], STICK, STREAM, TABNDX, TALLY, THRESH,
    THROW, TK[21], TRAVEL[886], TRIDNT, TRNDEX, TRNLUZ, TRNSIZ = 5, TRNVAL[6],
    TRNVLS, TROLL, TROLL2, TRVS, TRVSIZ = 885, TTEXT[6], TURNS, URN, V1, V2,
    VASE, VEND, VERB, VOLCAN, VRBSIZ = 35, VRSION = 25, WATER, WD1, WD1X, WD2,
...
int main(void) {
  ...
  /*  HISTORY: ORIGINAL IDEA & 5-TREASURE VERSION (ADVENTURES) BY WILLIE
   * CROWTHER 15-TREASURE VERSION (ADVENTURE) BY DON WOODS, APRIL-JUNE 1977
...
L2:
   ...
   * SO, THE DWARF'S BLOCKING HIS WAY.  IF COMING FROM PLACE FORBIDDEN TO PIRATE
...
L74:
   ...
   *  THE WHOLE MESS THE FIRST TIME HE GETS AS FAR AS THE HALL OF MISTS (LOC
   * 15). IF NEWLOC IS FORBIDDEN TO PIRATE (IN PARTICULAR, IF IT'S BEYOND THE
   * TROLL BRIDGE), BYPASS DWARF STUFF.  THAT WAY PIRATE CAN'T STEAL RETURN
   * TOLL, AND DWARVES CAN'T MEET THE BEAR.  ALSO MEANS DWARVES WON'T FOLLOW HIM
   * INTO DEAD END IN MAZE, BUT C'EST LA VIE.  THEY'LL WAIT FOR HIM OUTSIDE THE
   * DEAD END. */
...
L6000:
  ...
  /*  THINGS ARE IN FULL SWING.  MOVE EACH DWARF AT RANDOM, EXCEPT IF HE'S SEEN
   * US HE STICKS WITH US.  DWARVES STAY DEEP INSIDE.  IF WANDERING AT RANDOM,
   *  THEY DON'T BACK UP UNLESS THERE'S NO ALTERNATIVE.  IF THEY DON'T HAVE TO
   *  MOVE, THEY ATTACK.  AND, OF COURSE, DEAD DWARVES DON'T DO MUCH OF
   * ANYTHING. */
...
L6010:
  ...
  /* 6030 */ for (I = 1; I <= 6; I++) {
    ...
    if (KK == 0)
      goto L6016;
  L6012:
    NEWLOC = MOD(IABS(TRAVEL[KK]) / 1000, 1000);
    {
      long x = J - 1;
      if (NEWLOC > 300 || !INDEEP(NEWLOC) || NEWLOC == ODLOC[I] ||
          (J > 1 && NEWLOC == TK[x]) || J >= 20 || NEWLOC == DLOC[I] ||
          FORCED(NEWLOC) || (I == 6 && CNDBIT(NEWLOC, 3)) ||
          IABS(TRAVEL[KK]) / 1000000 == 100)
        goto L6014;
    }
    TK[J] = NEWLOC;
    J = J + 1;
  ...
  L6016:
     ...
     *  COUNTS IF A TREASURE IS HERE.  IF NOT, AND TALLY=1 FOR AN UNSEEN CHEST,
     * LET THE PIRATE BE SPOTTED.  NOTE THAT PLACE(CHEST)=0 MIGHT MEAN THAT HE'S
    ...
    if (TALLY == 1 && K == 0 && PLACE[CHEST] == 0 && HERE(LAMP) &&
        PROP[LAMP] == 1)
      goto L6025;
...
L8000:
   ...
   * HE DIES.  (IF HE DOES, NEWLOC WILL BE LIMBO, AND OLDLOC WILL BE WHAT KILLED
...
L30000:
   ...
   * NEVER BE USED FOR ACTUAL MOTION, BUT CAN BE SPOTTED BY "GO BACK". */
...
L30100:
  ...
  /*  TRAVEL 302.  PLOVER TRANSPORT.  DROP THE EMERALD (ONLY USE SPECIAL TRAVEL
   * IF TOTING IT), SO HE'S FORCED TO USE THE PLOVER-PASSAGE TO GET IT OUT.
...
L30200:
   ...
   *  DWARVES WON'T WANDER ACROSS AND ENCOUNTER THE BEAR.  (THEY WON'T FOLLOW
   * THE PLAYER THERE BECAUSE THAT REGION IS FORBIDDEN TO THE PIRATE.)  IF
...
L21:
  ...
  if (LL > 300)
    goto L22;
  J = KEY[LL];
  if (FORCED(LL) && MOD((IABS(TRAVEL[J]) / 1000), 1000) == K)
    K2 = KK;
L22:
  if (TRAVEL[KK] < 0)
    goto L23;
...
L25:
   ...
   * "NOW" BE DARK) SO HE WON'T FALL INTO A PIT WHILE STARING INTO THE GLOOM. */
...
L50:
   ...
   * ETC.) WHICH OFFERS REINCARNATION; IF ACCEPTED, THIS RESULTS IN MESSAGE 82,
   * 84, ETC.  THE LAST TIME, IF HE WANTS ANOTHER CHANCE, HE GETS A SNIDE REMARK
   * AS WE EXIT.  WHEN REINCARNATED, ALL OBJECTS BEING CARRIED GET DROPPED AT
   * OLDLC2 (PRESUMABLY THE LAST PLACE PRIOR TO BEING KILLED) WITHOUT CHANGE OF
   * PROPS. THE LOOP RUNS BACKWARDS TO ASSURE THAT THE BIRD IS DROPPED BEFORE
   * THE CAGE. (THIS KLUGE COULD BE CHANGED ONCE WE'RE SURE ALL REFERENCES TO
   * BIRD AND CAGE ARE DONE BY KEYWORDS.)  THE LAMP IS A SPECIAL CASE (IT
   * WOULDN'T DO TO LEAVE IT IN THE CAVE).  IT IS TURNED OFF AND LEFT OUTSIDE
   * THE BUILDING (ONLY IF HE WAS CARRYING IT, OF COURSE).  HE HIMSELF IS LEFT
   * INSIDE THE BUILDING (AND HEAVEN HELP HIM IF HE TRIES TO XYZZY BACK INTO THE
   * CAVE WITHOUT THE LAMP!). OLDLOC IS ZAPPED SO HE CAN'T JUST "RETREAT". */
...
L99:
  if (CLOSNG)
    goto L95;
  NUMDIE = NUMDIE + 1;
  if (!YES(79 + NUMDIE * 2, 80 + NUMDIE * 2, 54))
    score(0);
  if (NUMDIE == MAXDIE)
    score(0);
  PLACE[WATER] = 0;
...
L95:
   ...
   * HINT. HINT NUMBER IS IN VARIABLE "HINT".  BRANCH TO QUICK TEST FOR
   * ADDITIONAL CONDITIONS, THEN COME BACK TO DO NEAT STUFF.  GOTO 40010 IF
   * CONDITIONS ARE MET AND WE WANT TO OFFER THE HINT.  GOTO 40020 TO CLEAR
...
L41000:
   ...
   * ZERO, WE BRANCH TO 10000 TO START CLOSING THE CAVE, AND THEN SIT BACK AND
   * WAIT FOR HIM TO TRY TO GET OUT.  IF HE DOESN'T WITHIN CLOCK2 TURNS, WE
   * CLOSE THE CAVE; IF HE DOES TRY, WE ASSUME HE PANICS, AND GIVE HIM A FEW
   * ADDITIONAL TURNS TO GET FRANTIC BEFORE WE CLOSE.  WHEN CLOCK2 HITS ZERO, WE
   * BRANCH TO 11000 TO TRANSPORT HIM INTO THE FINAL PUZZLE.  NOTE THAT THE
   * PUZZLE DEPENDS UPON ALL SORTS OF RANDOM THINGS.  FOR INSTANCE, THERE MUST
   * BE NO WATER OR OIL, SINCE THERE ARE BEANSTALKS WHICH WE DON'T WANT TO BE
   * ABLE TO WATER, SINCE THE CODE CAN'T HANDLE IT.  ALSO, WE CAN HAVE NO KEYS,
   * SINCE THERE IS A GRATE (HAVING MOVED THE FIXED OBJECT!) THERE SEPARATING
   * HIM FROM ALL THE TREASURES.  MOST OF THESE PROBLEMS ARISE FROM THE USE OF
   * NEGATIVE PROP NUMBERS TO SUPPRESS THE OBJECT DESCRIPTIONS UNTIL HE'S
   * ACTUALLY MOVED THE OBJECTS. */
...
L10000:
   ...
   *  OBJECTS COME FROM KNOWN LOCATIONS AND/OR STATES (E.G. THE SNAKE IS KNOWN
   * TO HAVE BEEN DESTROYED AND NEEDN'T BE CARRIED AWAY FROM ITS OLD "PLACE"),
