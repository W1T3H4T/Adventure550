...
L380:
  ...
  {
    long x = NPARMS + 1;
    PUTTXT(PARMS[x], STATE, CASE, 0);
  }
  if (PRMTYP == 13 && INLINE[I] >= 37 && INLINE[I] <= 62)
    INLINE[I] = INLINE[I] - 26;
  NPARMS = NPARMS + 2;
  goto L32;
...
void fGETIN(long *wORD1, long *wORD1X, long *wORD2, long *wORD2X) {
   ...
   *  BLANKS, AND RETURN IT IN WORD1.  CHARS 6 THRU 10 ARE RETURNED IN WORD1X,
   * IN CASE WE NEED TO PRINT OUT THE WHOLE WORD IN AN ERROR MESSAGE.  ANY
   * NUMBER OF BLANKS MAY FOLLOW THE WORD.  IF A SECOND WORD APPEARS, IT IS
   * RETURNED IN WORD2 (CHARS 6 THRU 10 IN WORD2X), ELSE WORD2 IS -1. */
...
#undef WORD2
#undef WORD2X
#define GETIN(WORD1, WORD1X, WORD2, WORD2X)                                    \
  fGETIN(&WORD1, &WORD1X, &WORD2, &WORD2X)
#undef YES
...
L30:
  LNPOSN = LNPOSN + 1;
L32:
  if (LNPOSN > LNLENG || INLINE[LNPOSN] == 0)
    goto L42;
...
L11:
  ...
  if (HASH)
    GETTXT = GETTXT + MOD(HASH * 13579L + 5432L, 97531L) * 12345L + HASH;
  return (GETTXT);
}
...
void fPUTTXT(long WORD, long *sTATE, long CASE, long HASH) {
  ...
  W = WORD;
  if (HASH)
    W = W - MOD(HASH * 13579L + 5432L, 97531L) * 12345L - HASH;
  /* 18 */ for (I = 1; I <= 5; I++) {
    if (W <= 0 && STATE == 0 && IABS(CASE) <= 1)
      return;
    BYTE = W / DIV;
    if (STATE != 0 || BYTE != 63)
      goto L12;
...
void fTYPE0(void) {
  ...
  /*  TYPE A BLANK LINE.  THIS PROCEDURE IS PROVIDED AS A CONVENIENCE FOR
   * CALLERS WHO OTHERWISE HAVE NO USE FOR MAPCOM. */
...
#undef SAVWDS
void fSAVWDS(long *W1, long *W2, long *W3, long *W4, long *W5, long *W6,
             long *W7) {
  ;
...
#define SAVWDS(W1, W2, W3, W4, W5, W6, W7)                                     \
  fSAVWDS(&W1, &W2, &W3, &W4, &W5, &W6, &W7)
#undef SAVARR
...
void fSAVWRD(long OP, long *wORD) {
   ...
   *  ALREADY OPEN, FINISH WRITING/READING IT AND DON'T START A NEW ONE.  IF
   * OP=0, READ/WRITE A SINGLE WORD.  WORDS ARE BUFFERED IN CASE THAT MAKES FOR
   * MORE EFFICIENT DISK USE.  WE ALSO COMPUTE A SIMPLE CHECKSUM TO CATCH
   * ELEMENTARY POKING WITHIN THE SAVED FILE.  WHEN WE FINISH READING/WRITING
   * THE FILE, WE STORE ZERO INTO WORD IF THERE'S NO CHECKSUM ERROR, ELSE
   * NONZERO. */
...
long fVOCAB(long ID, long INIT) {
   ...
   * CONSIDERED. (THUS "STEPS", WHICH IS A MOTION VERB AS WELL AS AN OBJECT, MAY
   * BE LOCATED AS AN OBJECT.)  AND IT ALSO MEANS THE KTAB VALUE IS TAKEN MOD
   * 1000. */
...
void fBUG(long NUM) {
  ...
  /*  THE FOLLOWING CONDITIONS ARE CURRENTLY CONSIDERED FATAL BUGS.  NUMBERS <
   *20 ARE DETECTED WHILE READING THE DATABASE; THE OTHERS OCCUR AT "RUN TIME".
...
void fMPINIT(void) {
  ...
  static long RUNS[7][2] = {32,  34, 39, 46, 65, 90, 97,
                            122, 37, 37, 48, 57, 0,  126};
  ...
  MAP1[128] = MAP1[10];
  /*  FOR THIS VERSION, TAB (9) MAPS TO SPACE (32), SO DEL (127) USES TAB'S
   * VALUE */
  MAP1[10] = MAP1[33];
  MAP1[11] = MAP1[33];
...
void fSAVEIO(long OP, long IN, long ARR[]) {
  ...
  /*  IF OP=0, ASK FOR A FILE NAME AND OPEN A FILE.  (IF IN=.TRUE., THE FILE IS
   * FOR INPUT, ELSE OUTPUT.)  IF OP>0, READ/WRITE ARR FROM/INTO THE
   * PREVIOUSLY-OPENED FILE.  (ARR IS A 250-INTEGER ARRAY.)  IF OP<0, FINISH
   * READING/WRITING THE FILE.  (FINISHING WRITING CAN BE A NO-OP IF A "STOP"
   * STATEMENT DOES IT AUTOMATICALLY.  FINISHING READING CAN BE A NO-OP AS LONG
   * AS A SUBSEQUENT SAVEIO(0,.FALSE.,X) WILL STILL WORK.)  IF YOU CAN CATCH
   * ERRORS (E.G., NO SUCH FILE) AND TRY AGAIN, GREAT.  DEC F40 CAN'T. */
