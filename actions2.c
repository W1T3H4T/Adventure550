...
L9023:
  ...
  PROP[CAVITY] = 0;
  if (!HERE(RUG) ||
      !((OBJ == EMRALD && PROP[RUG] != 2) || (OBJ == RUBY && PROP[RUG] == 2)))
    goto L9021;
  SPK = 219;
  if (TOTING(RUG))
    SPK = 220;
...
L9126:
   ...
   * DEAD, MOVE DRAGON TO CENTRAL LOC (STILL FIXED), MOVE RUG THERE (NOT FIXED),
   * AND MOVE HIM THERE, TOO.  THEN DO A NULL MOTION TO GET NEW DESCRIPTION. */
  RSPEAK(49);
  VERB = 0;
  OBJ = 0;
