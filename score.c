...
L20000:
   ...
   *  TELL US IF HE EVER GOT SUITABLY DEEP INTO THE CAVE.  CLOSNG STILL
   * INDICATES WHETHER HE REACHED THE ENDGAME.  AND IF HE GOT AS FAR AS "CAVE
   * CLOSED" (INDICATED BY "CLOSED"), THEN BONUS IS ZERO FOR MUNDANE EXITS OR
   * 133, 134, 135 IF HE BLEW IT (SO TO SPEAK). */
...
L20020:
  ...
  if (SCORE + TRNLUZ + 1 >= MXSCOR && TRNLUZ != 0)
    RSPEAK(242);
  if (SCORE + SAVED + 1 >= MXSCOR && SAVED != 0)
    RSPEAK(143);
